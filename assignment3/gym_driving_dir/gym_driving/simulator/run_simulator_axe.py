...
class Task1():
    ...
    def next_action(self, state):
        ...
        x, y, vel, angle = state
        angle_needed=math.atan2((-y),(350-x))*180/math.pi
        ...
        diff = angle - angle_needed
        if(abs(diff) <= 3 or abs(diff) >= 357):
            action_steer = 1
            action_acc = 4
...
class Task2():
    ...
    def next_action(self, state):
        ...
        # Replace with your implementation to determine actions to be taken
        x, y, vel, angle = state
        if (abs(y) < 30): # go straight right if near x axis
        ...
        for cen in self.cen_list:
            cx, cy = cen[0], cen[1]
            # check if any pit is blocking path to x axis 
            if(x<=cx+75 and x>=cx-75):
                if (y>0 and ((y>=cy and cy>0))or (y<0 and (y<=cy and cy<0))):
                    if(angle <= 5 or angle >= 355):
                        action_steer = 1
                        action_acc = 4
        ...
        # reached here means no pit is blocking path to x axis 
        # so move vertically until reaching x axis
        if y > 0:
            if abs(angle - 270) <= 3:
                action_steer, action_acc = 1,4
            else:
                action_steer, action_acc =  0 if (angle < 90 or angle > 270) else 2,0
        else:
            if abs(angle - 90) <= 3:
                action_steer, action_acc = 1,4
            else:
                action_steer, action_acc = 2 if (angle < 90 or angle > 270) else 0,0
        action = np.array([action_steer, action_acc])  
        return action    
