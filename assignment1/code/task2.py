...
class AlgorithmBatched:
    ...
    def give_pull(self):
        # START EDITING HERE
        def arm_index(s,f):
            beta_samples = [np.random.beta(s[i]+1,f[i]+1) for i in range(self.num_arms)]
            return np.argmax(beta_samples)     
           
        all_arms = [arm_index(self.successes,self.failures) for _ in range(self.batch_size)]
        unique_arms,arm_counts = np.unique(all_arms,return_counts=True)

        return unique_arms,arm_counts
        # END EDITING HERE
    
    def get_reward(self, arm_rewards):
        # START EDITING HERE
        for arm_index in arm_rewards:
            rewards_list = arm_rewards[arm_index]
            s = (rewards_list == 1).sum()
            f = (rewards_list == 0).sum()
            self.successes[arm_index] += s
            self.failures[arm_index] += f    
        # END EDITING HERE